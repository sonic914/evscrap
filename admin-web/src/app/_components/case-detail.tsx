      ...
      <div className="page">
        ...
        {caseData?.lots && caseData.lots.length > 0 && (
          <div style={{ marginTop: 24 }}>
            <h2>부품 (Lots)</h2>
            <table>
              <thead><tr><th>Lot ID</th><th>부품 타입</th><th>수량</th><th>무게(kg)</th><th>생성일</th></tr></thead>
              <tbody>
                {caseData.lots.map((lot) => (
                  <tr key={lot.lot_id}>
                    <td style={{ fontFamily: 'monospace', fontSize: 11 }}>{lot.lot_id.slice(0, 8)}…</td>
                    <td>{lot.part_type}</td><td>{lot.quantity}</td><td>{lot.weight_kg ?? '-'}</td>
        ...
        <div style={{ marginTop: 24 }}>
          ...
          {timelineLoading ? (
            ...
            <table>
              <thead><tr><th>시간</th><th>이벤트 타입</th><th>앵커 상태</th><th>Hash</th><th>Event ID</th></tr></thead>
              <tbody>
                {timeline.map((ev) => (
                  <tr key={ev.event_id}>
                    <td>{new Date(ev.occurred_at).toLocaleString('ko')}</td>
                    <td>{ev.event_type}</td><td>{anchorBadge(ev.anchor_status)}</td>
        ...
        <div style={{ marginTop: 24 }}>
          ...
          {evidenceLoading ? (
            ...
            <table>
              <thead><tr><th>Evidence ID</th><th>파일 타입</th><th>크기</th><th>업로드일</th><th>열기</th></tr></thead>
              <tbody>
                {evidence.map((ev) => (
                  <tr key={ev.evidence_id}>
                    <td style={{ fontFamily: 'monospace', fontSize: 11 }}>{ev.evidence_id.slice(0, 8)}…</td>
                    <td>{ev.mime_type}</td><td>{formatBytes(ev.size_bytes)}</td>
                    <td>{new Date(ev.uploaded_at).toLocaleDateString('ko')}</td>
                    <td>{ev.presigned_url ? <a href={ev.presigned_url} target="_blank" rel="noopener noreferrer">Open ↗</a> : <span style={{ color: '#999', fontSize: 12 }}>URL 미제공</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
