    ...
    try {
        ...
        if (unverifiedEvents.length > 0) {
            return res.status(409).json({
                error: 'ANCHOR_NOT_VERIFIED',
                message: `${unverifiedEvents.length} event(s) not yet verified. All events must be anchor-verified before approval.`,
                unverified_events: unverifiedEvents.map(e => ({
                    event_id: e.id,
        ...
        if (eventCount === 0) {
            return res.status(409).json({
                error: 'NO_EVENTS',
                message: 'No events found for this target. At least one verified event is required.',
            });
        }
        ...
        if (settlement.status !== 'DRAFT' && settlement.status !== 'READY_FOR_APPROVAL') {
            return res.status(409).json({
                ...
                message: `Cannot approve settlement in '${settlement.status}' status. Must be DRAFT or READY_FOR_APPROVAL.`,
            });
        }
        ...
        return res.status(500).json({ error: 'InternalServer', message: 'Failed to approve settlement' });
    }
};
...
export const commitSettlement = async (req: Request, res: Response) => {
    try {
        ...
        if (unverifiedEvents.length > 0) {
            return res.status(409).json({
                error: 'ANCHOR_NOT_VERIFIED',
                message: `${unverifiedEvents.length} event(s) not yet verified. All events must be anchor-verified before commit.`,
                unverified_events: unverifiedEvents.map(e => ({
                    event_id: e.id,
                    event_type: e.eventType,
                    anchor_status: e.anchorStatus,
        ...
        if (settlement.status !== 'APPROVED') {
            return res.status(409).json({
                error: 'INVALID_STATUS_TRANSITION',
                message: `Cannot commit settlement in '${settlement.status}' status. Must be APPROVED first.`,
            });
        }
        ...
        if (!receipt_hash) {
            return res.status(400).json({
                ...
                message: 'receipt_hash is required for commit.',
            });
        }
        ...
        return res.status(500).json({ error: 'InternalServer', message: 'Failed to commit settlement' });
    }
};
